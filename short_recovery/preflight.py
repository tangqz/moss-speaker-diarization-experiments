...
SOURCE_BINDING_FILES = (
    ...
    "calibrate.py", "finalize_calibration.py", "config.default.json",
    "config.train150.proposed.json", "fsdp1_offload.json", "train.slurm",
    "train150_pipeline.slurm",
    "config.error-repeat.proposed.json", "pipeline_error_repeat.slurm",
    "../repeated_error_recovery/evaluate_milestone.py",
    "../repeated_error_recovery/status.py",
)
...
def expected_bindings(config, manifest_summary: dict) -> dict:
    ...
    if config.arm in {"sub", "mix", "sub_repeat"}:
        ...
        if not calibration.is_file():
            raise FileNotFoundError("sparse arm gate requires the Dev calibration artifact")
        bindings["calibration_sha256"] = file_sha256(calibration)
    return bindings
