...
def update_sample(model):
    ...
    for name,p in model.named_parameters():
        if name not in selected:
            continue
        count = min(65536,p.numel())
        # Integer arithmetic avoids float32 rounding numel-1 up to numel for
        # large embedding tensors (an out-of-bounds diagnostic index).
        idx = torch.arange(count,device=p.device,dtype=torch.long)*(p.numel()-1)//max(count-1,1)
        assert int(idx[-1]) < p.numel()
