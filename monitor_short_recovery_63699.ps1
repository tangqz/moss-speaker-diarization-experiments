...
$remoteLog = '/work/qt28/moss/logs/short-recovery-sub150-63699.out'
$remoteErr = '/work/qt28/moss/logs/short-recovery-sub150-63699.err'

Set-Location -LiteralPath $workspace
while ($true) {
    Clear-Host
    Write-Host ('MOSS short-recovery sub 150 | job 63699 | ' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'))
    python $sshHelper --command "squeue -j 63699 -o '%.18i %.24j %.2t %.10M %R'"
    ...
    $state = python $sshHelper --command "squeue -h -j 63699 -o '%T'"
    if (-not $state) {
