...
def build_swift_args(config, manifest: Path, output_dir: Path, resume: str | None) -> list[str]:
    args = [
        ...
        "--template", "moss_local_ul30", "--tuner_type", "full",
...
def main():
    ...
    if args.resume:
        raise RuntimeError("local-ul30 is a fresh-optimizer Base run; resume is forbidden")
    ...
    if engineering:
        output_dir = (Path(config.output_root) / "local-ul30-engineering" / args.run_id /
                      args.engineering_stage / config.arm / f"seed-{config.seed}")
    else:
        output_dir = Path(config.output_root) / f"local-ul30-{args.run_id}" / config.arm / f"seed-{config.seed}"
