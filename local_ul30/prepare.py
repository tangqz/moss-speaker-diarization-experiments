...
def build_manifest(config_path: Path, output_dir: Path, source_limit: int | None = None) -> dict[str, Any]:
    ...
    for update, start in enumerate(range(0, len(prepared), 4)):
        ...
        # Deterministic fixed event shared byte-for-byte by B/C: the legal
        # anchor token remains and two or three copies are inserted.
        error_repeat_counts = {0: 2, 1: 2, 2: 3, 3: 3}
        for slot, item in enumerate(group):
            ...
            if config.arm != "ce":
                ...
                elif config.arm in {"recovery", "ul"}:
                    kinds.append("repeat_previous")
            for view, kind in enumerate(kinds):
                repeat_count = error_repeat_counts.get(slot, 0)
                meta = {
                    "schema_version": "local-ul30-row-v1", "arm": config.arm,
            ...
            "lambda": budget.lambda_value, "clean_scale": budget.clean_scale,
                    "aux_scale": budget.aux_scale,
                    "aux_exit_scale": (budget.lambda_value * budget.total / 4
                                        if config.arm in {"recovery", "ul"} else 0.0),
                    "aux_tail_scale": (budget.lambda_value * budget.total / (4 * 15)
                                        if config.arm in {"recovery", "ul"} else 0.0),
    ...
    summary = {"schema_version": "local-ul30-manifest-v1", "arm": config.arm,
...
def derive_arm_manifest(source_summary_path: Path, config_path: Path, output_dir: Path) -> dict[str, Any]:
    """Derive the second loss arm without re-reading audio or re-tokenizing text."""
    config = load_config(config_path)
    source_summary = json.loads(source_summary_path.read_text(encoding="utf-8"))
    if source_summary.get("status") != "pass":
        raise ValueError("source manifest summary is not passing")
    source_manifest = Path(source_summary["manifest"])
    if file_sha256(source_manifest) != source_summary.get("manifest_sha256"):
        raise ValueError("source manifest hash mismatch")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_manifest = output_dir / f"train.{config.arm}.jsonl"
    rows = []
    with source_manifest.open(encoding="utf-8") as handle:
        for line in handle:
            if not line.strip():
                continue
            row = json.loads(line)
            meta = row.get("chat_template_kwargs", {}).get("recovery_meta")
            if not isinstance(meta, dict) or meta.get("schema_version") != "local-ul30-row-v1":
                raise ValueError("source manifest contains a non-local-ul30 row")
            if meta.get("arm") not in {"recovery", "ul"}:
                raise ValueError("source manifest arm is not recovery/ul")
            meta["arm"] = config.arm
            row["chat_template_kwargs"]["recovery_meta"] = meta
            rows.append(row)
    output_manifest.write_text("".join(json.dumps(row, ensure_ascii=False, separators=(",", ":")) + "\n"
                                   for row in rows), encoding="utf-8")
    for name in ("legal_body_ids.json",):
        source = source_manifest.parent / name
        if source.is_file():
            (output_dir / name).write_bytes(source.read_bytes())
    summary = dict(source_summary)
    summary.update({"schema_version": "local-ul30-manifest-v1", "arm": config.arm,
                    "manifest": str(output_manifest), "manifest_sha256": file_sha256(output_manifest),
                    "config_sha256": config.digest,
                    "derived_from_manifest_sha256": source_summary["manifest_sha256"],
                    "status": "pass"})
    (output_dir / "manifest_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    return summary
...
def main() -> None:
    ...
    derive = sub.add_parser("derive-arm")
    derive.add_argument("--source-summary", required=True, type=Path)
    derive.add_argument("--config", required=True, type=Path)
    derive.add_argument("--output-dir", required=True, type=Path)
    args = parser.parse_args()
    if args.command == "tokenizer-audit":
        result = tokenizer_audit(args.manifest, args.tokenizer_json, args.output)
    elif args.command == "manifest":
        result = build_manifest(args.config, args.output_dir, args.source_limit)
    else:
        result = derive_arm_manifest(args.source_summary, args.config, args.output_dir)
