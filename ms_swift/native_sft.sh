#!/usr/bin/env bash
set -euo pipefail
TASK=${MOSS_TASK_ROOT:-/work/qt28/moss/dkucc/ms_swift_20260914}
...
swift sft \
  ...
  --model_type moss_transcribe_diarize --template moss_transcribe_diarize \
  --external_plugins "$TASK/moss_plugin.py" "$TASK/runtime_callback.py" \
  ...
  --learning_rate 1e-6 --lr_scheduler_type linear --warmup_ratio 0 --weight_decay 0 \
  ...
  --ddp_find_unused_parameters false --logging_steps 1 --save_steps 5 \
  --eval_strategy no --prediction_loss_only true --report_to tensorboard --callbacks moss_phase \
